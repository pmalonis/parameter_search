...
double spike_height(const std::vector<double> v);
double spike_width(const std::vector<double> v);

double spkd(std::vector<int> spikes_1, std::vector<int> spikes_2, double cost); 
...
int main(int argc, char** argv)
{
    ...
    int row_size = nparams + 4; //there are four distance metrics
                                //between the model and the recording
    ...
    for (int j = 0; j < n_sets; j++) {
        if (j%nthreads == tid) {
            ...
            cross_correlation = max_xcorr(voltage_model_end, 
                                          voltage_recorded_end,
                                          nlines, max_offset);

            mean_width_model = spike_width(voltage_model);
            mean_height_model = spike_height(voltage_model);
            
            //copying metrics to data buffer
            row_data[nparams] = spike_distance;
            row_data[nparams + 1] = cross_correlation;
            row_data[nparams + 2] = abs(mean_width_model - mean_width_recorded); 
            row_data[nparams + 3] = abs(mean_height_model - mean_height_recorded); 
            //writing row
